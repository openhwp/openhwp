...
pub use track_change_author::*;

use crate::{DocInfoIter, HwpDocumentError};
use crate::{HwpTag, u32};
...
pub struct IdMappings {
    ...
    pub track_changes: Vec<TrackChangeEntry>,
...
impl<'hwp> DocInfoIter<'hwp> {
    pub fn id_mappings(&mut self) -> Result<IdMappings, HwpDocumentError> {
        ...
        Ok(IdMappings {
            bin_data: self.bin_data(&id_mapping_count)?,
            face_names: self.face_names(&id_mapping_count)?,
            border_fills: self.border_fills(&id_mapping_count),
            char_shapes: self.char_shapes(&id_mapping_count),
            tab_definitions: self.tab_definitions(&id_mapping_count),
            numberings: self.numberings(&id_mapping_count),
            bullets: self.bullets(&id_mapping_count),
            paragraph_shapes: self.paragraph_shapes(&id_mapping_count),
            styles: self.styles(&id_mapping_count),
            memo_shapes: self.memo_shapes(&id_mapping_count),
            track_change_authors: self.track_change_authors(&id_mapping_count),
            track_changes: self.track_change_entries(&id_mapping_count),
            doc_data: self.doc_data(),
            distribute_doc_data: self.distribute_doc_data(),
